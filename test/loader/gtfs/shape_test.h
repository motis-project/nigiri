...
class shape_test_mmap {
  ...
  static mm_vecvec<Key, Value> create_mmap(
...
public:
  shape_test_mmap(std::string base_path) : paths{create_paths(base_path)}, mmap{create_mmap(paths)} {}
  ~shape_test_mmap() {
    for (auto path : paths) {
  ...
  mm_vecvec<Key, Value>& get_vecvec() {
    return mmap;
  }
private:
  std::vector<std::string> paths;
