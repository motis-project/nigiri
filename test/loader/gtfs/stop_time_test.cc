...
TEST(loader_gtfs_route, read_stop_times_example_data) {
  ...
  auto trips_ = read_trips(routes, services, files.get_file(kTripsFile).data());
  auto& [trips, blocks, trip_data] = trips_;
  auto const stops = read_stops(source_idx_t{0}, tt, timezones,
                                files.get_file(kStopFile).data(),
                                files.get_file(kTransfersFile).data());

  read_stop_times(trips_, stops, files.get_file(kStopTimesFile).data());

  for (auto& t : trip_data) {
  ...
  auto& awe1_stop_times = trip_data[awe1_it->second].event_times_[0];
  auto stop = timetable::stop{trip_data[awe1_it->second].stop_seq_[0]};
  ...
  awe1_stop_times = trip_data[awe1_it->second].event_times_[1];
  stop = timetable::stop{trip_data[awe1_it->second].stop_seq_[1]};
  ...
  awe1_stop_times = trip_data[awe1_it->second].event_times_[2];
  stop = timetable::stop{trip_data[awe1_it->second].stop_seq_[2]};
  ...
  awe1_stop_times = trip_data[awe1_it->second].event_times_[3];
  stop = timetable::stop{trip_data[awe1_it->second].stop_seq_[3]};
  ...
  awe1_stop_times = trip_data[awe1_it->second].event_times_[4];
  stop = timetable::stop{trip_data[awe1_it->second].stop_seq_[4]};
  ...
  read_frequencies(trips_, files.get_file(kFrequenciesFile).data());
}
