...
TEST(gtfs, read_trips_example_data) {
  ...
  auto const& [trips, blocks, trips_data] = trips_;

  EXPECT_EQ(2U, trips.size());
  EXPECT_NE(end(trips), trips.find("AWE1"));
  EXPECT_EQ("A", trips_data.at(trips.at("AWE1")).route_->id_);
  EXPECT_EQ("Downtown", trips_data.at(trips.at("AWE1")).headsign_);
}
...
TEST(gtfs, read_trips_berlin_data) {
  ...
  EXPECT_EQ("1", trips_data[trips.at("1")].route_->id_);
  EXPECT_EQ("Flughafen Schönefeld Terminal (Airport)",
            trips_data[trips.at("1")].headsign_);

  EXPECT_NE(end(trips), trips.find("2"));
  EXPECT_EQ("1", trips_data[trips.at("2")].route_->id_);
  EXPECT_EQ("S Potsdam Hauptbahnhof", trips_data[trips.at("2")].headsign_);

  EXPECT_NE(end(trips), trips.find("3"));
  EXPECT_EQ("2", trips_data[trips.at("3")].route_->id_);
  EXPECT_EQ("Golzow (PM), Schule", trips_data[trips.at("3")].headsign_);
}
