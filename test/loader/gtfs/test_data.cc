...
constexpr auto const test_calendar_file_content = std::string_view{
    R"(service_id,monday,tuesday,wednesday,thursday,friday,saturday,sunday,start_date,end_date
X,0,0,0,0,0,0,0,20190315,20191115
)"};

constexpr auto const test_calendar_dates_file_content =
    R"(service_id,date,exception_type
X,20190331,1
X,20191027,1
)";

constexpr auto const test_stops_file_content = std::string_view{
    R"(stop_id,stop_name,stop_desc,stop_lat,stop_lon,stop_url,location_type,parent_station
A,A,,0.0,1.0,,
B,B,,2.0,3.0,,
C,C,,4.0,5.0,,
...
constexpr auto const test_agency_file_content = std::string_view{
    R"(agency_id,agency_name,agency_url,agency_timezone
DB,Deutsche Bahn,https://deutschebahn.com,Europe/Berlin
)"};

auto const test_routes_file_content = std::string_view{
    R"(route_id,agency_id,route_short_name,route_long_name,route_desc,route_type
A,DB,1337,Long Name,Route Description,3
)"};

constexpr auto const test_transfers_file_content =
    std::string_view{R"(from_stop_id,to_stop_id,transfer_type,min_transfer_time
)"};

constexpr auto const test_trips_file_content =
    R"(route_id,service_id,trip_id,trip_headsign,block_id
A,X,X1,Trip X,
)";

constexpr auto const test_frequencies_file_content =
    std::string_view{R"(trip_id,start_time,end_time,headway_secs
)"};

constexpr auto const test_stop_times_content =
    R"(trip_id,arrival_time,departure_time,stop_id,stop_sequence,pickup_type,drop_off_type
X1,00:00:00,00:00:00,A,1,0,0
X1,00:59:00,01:00:00,B,2,0,0
X1,02:00:00,02:01:00,C,3,0,0
X1,02:59:00,03:00:00,D,4,0,0
X1,03:30:00,03:30:00,E,5,0,0
)";

loader::mem_dir test_files() {
  using std::filesystem::path;
  return {{{path{kAgencyFile}, std::string{test_agency_file_content}},
           {path{kStopFile}, std::string{test_stops_file_content}},
           {path{kCalenderFile}, std::string{test_calendar_file_content}},
           {path{kCalendarDatesFile},
            std::string{test_calendar_dates_file_content}},
           {path{kTransfersFile}, std::string{test_transfers_file_content}},
           {path{kRoutesFile}, std::string{test_routes_file_content}},
           {path{kFrequenciesFile}, std::string{test_frequencies_file_content}},
           {path{kTripsFile}, std::string{test_trips_file_content}},
           {path{kStopTimesFile}, std::string{test_stop_times_content}}}};
}
