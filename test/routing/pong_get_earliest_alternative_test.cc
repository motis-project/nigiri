...
// Block-concatenated trip: T_NO_BIKE (A→B, bikes_allowed=2) followed by
// T_BIKE (B→C, bikes_allowed=1) sharing the same block_id. Nigiri loads
// these as a single block-trip on a route with stop sequence A→B→C and a
// per-section bikes_allowed pattern of {false, true}. `has_bike_transport`
// returns true (some section allows bikes), but a journey A→C requires
// bikes through the A→B section which is not allowed.
constexpr auto const kBikeBlockGTFS = R"(
# agency.txt
...
// Same idea for cars: T_CAR (A→B, cars_allowed=1) followed by T_NO_CAR
// (B→C, cars_allowed=2). A→C requires cars on the B→C section which is
// not allowed.
constexpr auto const kCarBlockGTFS = R"(
# agency.txt
agency_id,agency_name,agency_url,agency_timezone
DB,DB,https://db.de,Europe/Berlin
...
// from X→A active at 10:55 (5min duration). The static footpath X→A is set
// to kMaxDuration to exclude it.
constexpr auto const kTdFpIngressGTFS = R"(
# agency.txt
agency_id,agency_name,agency_url,agency_timezone
DB,DB,https://db.de,Europe/Berlin
...
// One transport A→B at 11:00; the egress B→Y requires a TD footpath active
// at 11:30 (5min duration). The static footpath B→Y is set to kMaxDuration
// to exclude it.
constexpr auto const kTdFpEgressGTFS = R"(
# agency.txt
agency_id,agency_name,agency_url,agency_timezone
DB,DB,https://db.de,Europe/Berlin
...
// FAILING: per-section bikes_allowed enforcement on block-concatenated
// trips. The block trip A→B→C has bikes_allowed_seq={false, true} (A→B
// without bikes, B→C with bikes). `tt.has_bike_transport(r)` returns true
// (some section allows bikes) so the new generator-based implementation
// includes this route — but the journey A→C requires bikes through both
// sections and must be rejected because A→B is bikeless.
TEST(pong, get_earliest_alternative_per_transport_bikes_allowed) {
  auto tt = timetable{};
  load(tt, kBikeBlockGTFS);
  ...
  // block A→B→C route was constructed).
  {
    ...
    auto const r_any = routing::get_earliest_alternative(
        tt, &rtt, q_any, A, C, from_arr, to_dep);
  ...
  auto const result = routing::get_earliest_alternative(
      tt, &rtt, q, A, C, from_arr, to_dep);

  EXPECT_FALSE(result.has_value())
      << "block trip A→B (no bikes) → B→C must be rejected for require_bike";
}

// FAILING: per-section cars_allowed on a block trip. Symmetric to the bike
// test: block trip A→B (cars) → B→C (no cars); journey A→C must be rejected
// for require_car_transport_=true.
TEST(pong, get_earliest_alternative_per_transport_cars_allowed) {
  auto tt = timetable{};
  load(tt, kCarBlockGTFS);
  auto const A = find_loc(tt, "A");
  auto const C = find_loc(tt, "C");
...
// FAILING: TD footpath at the ingress side (q.prf_idx_ != 0).
...
// A at 11:00. Current implementation only consults static footpaths and
// therefore returns nullopt.
TEST(pong, get_earliest_alternative_td_footpath_ingress) {
  constexpr auto const kProfile = profile_idx_t{2U};
  auto tt = timetable{};
  load(tt, kTdFpIngressGTFS);
  ...
  auto const result = routing::get_earliest_alternative(
      tt, &rtt, q, X, B,
      unixtime_t{sys_days{2024_y / June / 19}} + 8h + 50min,
...
// FAILING: TD footpath at the egress side (q.prf_idx_ != 0).
// Egress B→Y only via a TD footpath active at 11:30 (5min). Static is
// kMaxDuration. Current implementation only consults static footpaths.
TEST(pong, get_earliest_alternative_td_footpath_egress) {
  constexpr auto const kProfile = profile_idx_t{2U};
  auto tt = timetable{};
  load(tt, kTdFpEgressGTFS);
