...
timetable load_gtfs(auto const& files) {
  timetable tt;
  tt.date_range_ = {sys_days{2021_y / March / 1},
...
void check_neighbours(tb_data const& tbd) {
  for(segment_idx_t i(0);i<tbd.segment_transports_.size();++i) {
    ...
    for(int j=0;j<end;++j) {
      ...
      EXPECT_TRUE(utl::any_of(tbd.segment_transfers_[i],[neighbour](transfer const& t){return t.to_segment_==neighbour;}));
...
TEST(tb_a_star,a_star_no_transfer) {
  ...
  a_star_exec.tt_ = tt;
  a_star_exec.travel_time_lower_bound_ = std::vector<uint16_t>(tt.n_locations(),0);
  ...
  a_star_exec.day_idx_[start_segment] = day_idx;//get_arrival_day(start_time,start_segment,tt,tbd);
  a_star_exec.state_.dist_to_dest_ = hash_map<segment_idx_t, duration_t>(tbd.segment_transports_.size());

  auto const it = a_star_exec.state_.dist_to_dest_.find(end_segment);
  if (it == end(a_star_exec.state_.dist_to_dest_)) {
    a_star_exec.state_.dist_to_dest_.emplace_hint(it, end_segment, duration_t::zero());
  } else {
    it->second = duration_t::zero();
  }

  pareto_set<journey> journeys;
  a_star_exec.execute(start_time,std::numeric_limits<std::uint8_t>::max(),unixtime_t::max(),profile_idx_t{0},journeys);
  ...
  a_star_exec.reconstruct(query{
    .start_time_ = start_time,
    .start_match_mode_ = location_match_mode::kEquivalent,
    .dest_match_mode_ = location_match_mode::kEquivalent,
    .start_ = {
            {tt.locations_.location_id_to_idx_.at({"S0", source_idx_t{0}}), 0_minutes, 0U}},
    .destination_ = {
            {tt.locations_.location_id_to_idx_.at({"S4", source_idx_t{0}}), 0_minutes, 0U}}},journey);
  EXPECT_EQ(journey.legs_.size(),2);
  EXPECT_FALSE(journey.error_);
  auto dest = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].back()}.location_idx();
  EXPECT_EQ(journey.dest_,dest);
  auto start = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].front()}.location_idx();
...
TEST(tb_a_star,a_star_with_transfer) {
  ...
  a_star_exec.reconstruct(query{
    ...
    .start_ = {
            {tt.locations_.location_id_to_idx_.at({"S0", source_idx_t{0}}), 0_minutes, 0U}},
    .destination_ = {
            {tt.locations_.location_id_to_idx_.at({"S3", source_idx_t{0}}), 0_minutes, 0U}}},journey);
  EXPECT_EQ(journey.legs_.size(),4);
  EXPECT_FALSE(journey.error_);
  auto dest = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{1}]].back()}.location_idx();
  EXPECT_EQ(journey.dest_,dest);
  auto start = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].front()}.location_idx();
  EXPECT_EQ(journey.legs_.front().from_,start);
  for(int i=0;i<journey.legs_.size()-1;++i) {
...
TEST(tb_a_star,a_star_longer_routes) {
  ...
  expected[3] = tbd.transport_first_segment_[transport_idx_t(1)]; // S3 to S5 in the next transport
  ...
  a_star_exec.reconstruct(query{
    ...
    .start_ = {
            {tt.locations_.location_id_to_idx_.at({"S0", source_idx_t{0}}), 0_minutes, 0U}},
    .destination_ = {
            {tt.locations_.location_id_to_idx_.at({"S7", source_idx_t{0}}), 0_minutes, 0U}}},journey);
  EXPECT_EQ(journey.legs_.size(),6);
  EXPECT_FALSE(journey.error_);
  auto dest = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{2}]].back()}.location_idx();
  EXPECT_EQ(journey.dest_,dest);
  auto start = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].front()}.location_idx();
  EXPECT_EQ(journey.legs_.front().from_,start);
  for(int i=0;i<journey.legs_.size()-1;++i) {
...
TEST(tb_a_star,overnight_travel) {
  ...
  a_star_exec.reconstruct(query{
    ...
    .start_ = {
            {tt.locations_.location_id_to_idx_.at({"S0", source_idx_t{0}}), 0_minutes, 0U}},
    .destination_ = {
            {tt.locations_.location_id_to_idx_.at({"S2", source_idx_t{0}}), 0_minutes, 0U}}},journey);
  EXPECT_EQ(journey.legs_.size(),4);
  EXPECT_FALSE(journey.error_);
  auto dest = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{1}]].back()}.location_idx();
  EXPECT_EQ(journey.dest_,dest);
...
TEST(tb_a_star,add_starts_one_segment) {
  ...
  auto start_segment = tbd.transport_first_segment_[transport_idx_t(0)];
  unixtime_t start_time = date::sys_days{date::year{2021}/date::March/date::day{1}}+10h;
...
TEST(tb_a_star,a_star_two_start_segments) {
  ...
  unixtime_t start_time = date::sys_days{date::year{2021}/date::March/date::day{1}}+10h;
  auto day_idx = tt.day_idx(date::year_month_day{date::year{2021},date::month{3},date::day{1}});
  tb_a_star a_star_exec(tbd.segment_transfers_.size());
  a_star_exec.state_.tbd_ = tbd;
  a_star_exec.tt_ = tt;
  a_star_exec.travel_time_lower_bound_ = std::vector<uint16_t>(tt.n_locations(),0);
  ...
  EXPECT_EQ(journey.legs_.size(),2);
  EXPECT_FALSE(journey.error_);
  auto dest = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{1}]][1]}.location_idx();
  EXPECT_EQ(journey.dest_,dest);
  auto start = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].front()}.location_idx();
  EXPECT_EQ(journey.legs_.front().from_,start);
  for(int i=0;i<journey.legs_.size()-1;++i) {
...
TEST(tb_a_star,search_execute) {
  ...
  routing::search<direction::kForward,tb_a_star> searcher{tt,nullptr,search_state,algo_state,std::move(q)};
  ...
  auto s3 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]][3]}.location_idx();
  auto s6 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{1}]][2]}.location_idx();
...
TEST(tb_a_star,search_overnight) {
  ...
  EXPECT_EQ(journey.legs_.back().to_,journey.dest_);
  auto s1 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].back()}.location_idx();
...
TEST(tb_a_star,search_intermodal) {
  ...
  EXPECT_EQ(journey.legs_.back().to_,get_special_station(special_station::kEnd));
  auto s0 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].front()}.location_idx();
  auto s3 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]][3]}.location_idx();
  auto s6 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{1}]][2]}.location_idx();
...
TEST(tb_a_star,search_different_termination) {
  ...
  EXPECT_EQ(result.els_.front().legs_.front().to_,result.els_.front().legs_.back().from_);
...
TEST(tb_a_star,search_max_cost) {
  ...
  EXPECT_FALSE(result.els_.front().error_);
  for(int i=0;i<3;++i) {
    EXPECT_EQ(result.els_.front().legs_[i].to_,result.els_.front().legs_[i+1].from_);
  }
  ...
  auto start = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]].front()}.location_idx();
  EXPECT_EQ(result.els_.front().legs_.front().from_,start);
  auto s1 = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]][1]}.location_idx();
...
TEST(tb_a_star,start_time_different_date_to_first_segment) {
  ...
  EXPECT_EQ(journey.legs_.front().dep_time_,sys_days{year{2021}/March/day{1}});
  EXPECT_EQ(journey.legs_.front().arr_time_,sys_days{year{2021}/March/day{1}}+20min);
  EXPECT_EQ(journey.legs_.back().arr_time_,sys_days{year{2021}/March/day{1}}+20min);
  EXPECT_EQ(journey.legs_.back().dep_time_,sys_days{year{2021}/March/day{1}}+20min);
...
TEST(tb_a_star,tranpsport_starts_before_start_time) {
  ...
  EXPECT_EQ(result.els_.front().legs_.back().to_,dest);
  EXPECT_EQ(result.els_.front().legs_.back().from_,dest);
  auto start = stop{tt.route_location_seq_[tt.transport_route_[transport_idx_t{0}]][1]}.location_idx();
  EXPECT_EQ(result.els_.front().legs_.front().from_,start);
  EXPECT_EQ(result.els_.front().legs_.front().to_,result.els_.front().legs_.back().from_);
}
} // namespace nigiri::routing::tb::a_star
