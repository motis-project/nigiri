...
[2024-06-19 07:00, 2024-06-19 09:12]
TRANSFERS: 0
     FROM: (START, START) [2024-06-19 07:00]
       TO: (END, END) [2024-06-19 09:12]
...
leg 2: (B, B) [2024-06-19 09:02] -> (END, END) [2024-06-19 09:12]
  MUMO (id=5, duration=10)
)";

TEST(routing, td_dest) {
  timetable tt;
  ...
  auto const run_search = [&]() {
    return raptor_search(
        ...
        routing::query{
            ...
            .td_dest_ =
                {{{B,
                   {{.valid_from_ = sys_days{2024_y / June / 19} + 9h + 2min,
                     .duration_ = 10min,
                     .transport_mode_id_ = 5},
                    {.valid_from_ = sys_days{2024_y / June / 19} + 9h + 3min,
  ...
  EXPECT_EQ(kExpected, to_string(tt, run_search()));
}