...
TEST_F(MeatCsaProfilesTest, SameAndCorrectOnSimpleRun) {
  ...
  ea_s = state_static_.profile_set_.compute_entry_amount();
  EXPECT_EQ(ea_s, 0);

  meat_dynamic_growth_.next_start_time();
  ea_dg = state_dynamic_growth_.profile_set_.compute_entry_amount();
  ne_dg = state_dynamic_growth_.profile_set_.n_entry_idxs();
  EXPECT_EQ(ea_dg, 0);
  EXPECT_EQ(ne_dg, 0);

  meat_dynamic_.next_start_time();
  ea_d = state_dynamic_.profile_set_.compute_entry_amount();
  ne_d = state_dynamic_.profile_set_.n_entry_idxs();
  EXPECT_EQ(ea_d, 0);
  EXPECT_EQ(ne_d, 0);
}
...
TEST_F(MeatCsaProfilesTest, SameOnNewMeatCsaRouter) {
  ...
  EXPECT_EQ(state_static_.profile_set_.compute_entry_amount(), 0);
  EXPECT_EQ(state_dynamic_growth_.profile_set_.compute_entry_amount(), 0);
  EXPECT_EQ(state_dynamic_.profile_set_.compute_entry_amount(), 0);
...
TEST_F(MeatCsaProfilesTest, SameProfileOnNewTT) {
  // auto g_s = meat_static_execute();
  // auto g_dg = meat_dynamic_growth_execute();
  // auto g_d = meat_dynamic_execute();
  ...
  auto meat_static = mcsa::meat_csa<mcsa::static_profile_set>{
      tt, state_static_, day_idx_t{4}, routing::all_clasz_allowed()};
  auto meat_dynamic_growth = mcsa::meat_csa<mcsa::dynamic_growth_profile_set>{
      tt, state_dynamic_growth_, day_idx_t{4}, routing::all_clasz_allowed()};
  auto meat_dynamic = mcsa::meat_csa<mcsa::dynamic_profile_set>{
      tt, state_dynamic_, day_idx_t{4}, routing::all_clasz_allowed()};
  EXPECT_EQ(state_static_.profile_set_.compute_entry_amount(), 0);
  EXPECT_EQ(state_dynamic_growth_.profile_set_.compute_entry_amount(), 0);
  EXPECT_EQ(state_dynamic_.profile_set_.compute_entry_amount(), 0);
  auto g_s2 = m::decision_graph{};
  ...
  auto const start_time =
      *tt.date_range_.begin();  // unixtime_t{sys_days{2024_y / March / 1}};
  auto const start_location =
  ...
  meat_static.execute(start_time, start_location, end_location, prf_idx, g_s2);
  auto ea_s2 = state_static_.profile_set_.compute_entry_amount();
  meat_dynamic_growth.execute(start_time, start_location, end_location, prf_idx,
                              g_dg2);
  auto ea_dg2 = state_dynamic_growth_.profile_set_.compute_entry_amount();
  meat_dynamic.execute(start_time, start_location, end_location, prf_idx, g_d2);
  auto ea_d2 = state_dynamic_.profile_set_.compute_entry_amount();

  expect_eq(g_s2, g_dg2);
  expect_eq(g_s2, g_d2);
  EXPECT_EQ(ea_s2, ea_dg2);
  EXPECT_EQ(ea_s2, ea_d2);
  expect_correct(g_s2, test_tt_expanded_dot_graph, tt);
}
