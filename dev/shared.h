...
inline auto get_cache_writer() {
  return get_cache(cista::mmap::protection::WRITE);
}

inline auto get_cache_reader() {
  return get_cache(cista::mmap::protection::READ);
}

inline auto get_mapper(cista::mmap::protection mode) {
  return cista::mmap_vec<id_type::value_type>{
      cista::mmap{id_map_file.data(), mode}};
}

inline auto get_map_reader() {
  return get_mapper(cista::mmap::protection::READ);
}

inline auto get_map_writer() {
