...
auto load_ids() {
  ...
  for (auto pos : std::vector<size_t>{0u, 1u, 756, 2763, 3659}) {
    std::cout << std::format("Position {:>5} contains {}", pos, ids.at(pos))
              << std::endl;
  }
...
auto load_shape_data() { return get_cache_reader(); }
...
void test_id_entries(std::vector<std::string> ids,
                     auto const& id_map,
                     auto const& shapes) {
  for (auto id : ids) {
    std::cout << std::format("Found {:>4} entries for id {}",
                             shapes.at(id_map.at(id)).size(), id)
              << std::endl;
  }
}
