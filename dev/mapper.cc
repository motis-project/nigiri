...
void test(const std::string& path) {
    ...
    if (exists) {
        std::cout << std::format("Size: {}", std::filesystem::file_size(path)) << std::endl;
    }
}
...
void fill(cista::mmap_vec<uint32_t>& map) {
    ...
    auto size = [&map]() { std::cout << "map size: " << map.size() << ", " << map.used_size_ << std::endl;};
    size();
    std::ranges::for_each(
            std::ranges::views::iota(0u, total)
...
void write(const auto& path) {
    ...
    mm_vecvec<std::size_t, datatype> map{
        cista::basic_mmap_vec<datatype, std::size_t>{cista::mmap{(path + ".values").data(), mode}},
        cista::basic_mmap_vec<std::size_t, std::size_t>{cista::mmap{(path + ".metadata").data(), mode}}
    };
...
void read(const auto& path) {
    ...
    const mm_vecvec<std::size_t, char> map{
        cista::basic_mmap_vec<char, std::size_t>{cista::mmap{(path + ".values").data(), mode}},
        cista::basic_mmap_vec<std::size_t, std::size_t>{cista::mmap{(path + ".metadata").data(), mode}}
    };
    std::cout << std::format("Size: {}", map.size()) << std::endl;
...
int main() {
    test(filepath);
    cista::mmap_vec<uint32_t> map{cista::mmap{filepath.data(), cista::mmap::protection::WRITE}};
    test(filepath);
    fill(map);
    test(filepath);
