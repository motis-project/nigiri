...
// reference to an implicit-transfer hub, packed like footpath (23-bit hub
// index, 9-bit duration): element of the transposed hub edge lists
// (timetable::locations::hub_*_by_loc_). all hub edge weights originate
// from footpaths or u8 transfer times, so the 9-bit range always fits.
struct hub_ref {
  hub_ref() = default;
  hub_ref(hub_idx_t const h, duration_t const duration)
      : hub_{to_idx(h)},
        duration_{static_cast<location_idx_t::value_t>(
            (duration > footpath::kMaxDuration ? footpath::kMaxDuration
                                               : duration)
                .count())} {
