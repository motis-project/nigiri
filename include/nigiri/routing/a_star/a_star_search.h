...
struct a_star_search {
  ...
  size_t cost_function(open_set_element el, unixtime_t arrival_time, uint32_t num_transfers_until_segment);

  void execute(unixtime_t start_time, uint8_t max_transfers, unixtime_t worst_time_at_dest, profile_idx_t prf_idx, pareto_set<journey>& journeys);

  void progress_neighbor_segment(tb::segment_idx_t current, tb::segment_idx_t neighbor, day_idx_t neighbor_day, bool is_same_trip);

  void expand_node(tb::segment_idx_t current);

  pair<location_idx_t, location_idx_t> get_segment_locations(tb::segment_idx_t seg);
  ...
  // open_set: all potential next segments (next segment in current trip or segments after transfer) as well as final footpaths
  // closed_set: all already covered segments
  // came_from: predecessor segment for each segment in openSet or closedSet
  // arrival_times, num_transfers_until_segment: relevant for cost function
  ...
  // 'start_segments' are the start segments which are actually viable, i.e. don't take too much time.
  // 'potential_starts' are the pairs of ALL start segments and the corresponding earliest transport.
