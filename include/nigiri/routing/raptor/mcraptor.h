...
// so only the extras (walk surcharge + boardings) are stored; dominance
// compares arr and arr + extras. Charging elapsed time bounds the
// arrival-vs-cost exchange rate: a label arriving dt minutes later must
// save more than dt weighted minutes of extras to stay on the frontier.
...
private:
  ...
  bool dest_dominates(unsigned const k, Criteria const& projected) const {
    ...
    for (auto const& e : dest_bag_) {
      if (e.k_ <= k && e.crit_.template dominates<SearchDir>(projected)) {
        return true;
      }
    }
    return false;
  ...
  void dest_bag_add(unsigned const k, Criteria const& crit) {
    ...
    for (auto i = std::size_t{0U}; i != dest_bag_.size(); ++i) {
      if (dest_bag_[i].k_ <= k &&
          dest_bag_[i].crit_.template dominates<SearchDir>(crit)) {
        return;
      }
      if (k <= dest_bag_[i].k_ &&
          crit.template dominates<SearchDir>(dest_bag_[i].crit_)) {
        ++removed;
        continue;
      }
