    ...
    struct mcraptor {
        ...
        void add_start(location_idx_t const l, unixtime_t const t) {
            ...
            new_best_[to_idx(l)][v] =
                get_best(unix_to_delta(base(), t), new_best_[to_idx(l)][v].time_);
            ...
            for (auto k = 1U; k != end_k; ++k) {
                for (auto i = 0U; i != n_locations_; ++i) {
                    for (auto v = 0U; v != Vias + 1; ++v) {
                        new_best_[i][v] = get_best(round_times_[k][i][v], new_best_[i][v].time_);
                    }
                }
                is_dest_.for_each_set_bit([&](std::uint64_t const i) {
                    update_time_at_dest(k, new_best_[i][Vias].time_);
                    });
            ...
            state_.prev_station_mark_.for_each_set_bit([&](auto&& i) {
                for (auto v = 0U; v != Vias + 1; ++v) {
                    ...
                    trace(
                        ...
                        "dest={}\n",
                        transfer_time, to_unix(fp_target_time), to_unix(new_best_[i][target_v].time_),
                    ...
                    if (!new_best_[i][target_v].is_better(fp_target_time) &&
                        ...
                        new_best_[i][target_v].time_ = fp_target_time;
                        ...
                        if (!new_best_[target][target_v].is_better(fp_target_time) &&
                            ...
                            if (lower_bound == kUnreachable ||
                                ...
                                trace_upd(
                                    ...
                                    k, loc{ tt_, l_idx }, to_unix(new_tmp_[to_idx(l_idx)][v].time_),
                                    adjusted_transfer_time(transfer_time_settings_,
                                        fp.duration()),
                                    loc{ tt_, fp.target() }, new_best_[target][target_v].time_,
                            ...
                            trace_upd(
                                ...
                                loc{ tt_, fp.target() }, to_unix(new_best_[target][target_v].time_),
                                to_unix(fp_target_time), v, target_v, stay);

                            ++stats_.n_earliest_arrival_updated_by_footpath_;
                            round_times_[k][target][target_v] = fp_target_time;
                            new_best_[target][target_v].time_ = fp_target_time;
                            ...
                            trace(
                                "┊ ├k={}   NO FP UPDATE: {} [best={}] --{}--> {} "
                                "[best={}, time_at_dest={}]\n",
                                k, loc{ tt_, l_idx }, to_unix(new_best_[to_idx(l_idx)][target_v].time_),
                                adjusted_transfer_time(transfer_time_settings_, fp.duration()),
                                loc{ tt_, fp.target() }, to_unix(new_best_[target][target_v].time_),
                                to_unix(time_at_dest_[k]));
                        }
                        ...
                        if (!new_best_[target][target_v].is_better(fp_target_time) &&
                            ...
                            if (lower_bound == kUnreachable ||
                                ...
                                trace_upd(
                                    ...
                                    "DEST={}\n",
                                    k, loc{ tt_, l_idx }, to_unix(new_tmp_[to_idx(l_idx)][v].time_),
                                    fp.duration(), loc{ tt_, fp.target() }, new_best_[target][target_v].time_,
                            ...
                            trace_upd(
                                ...
                                to_unix(new_best_[target][target_v].time_), to_unix(fp_target_time), v,
                                target_v, stay);
                            ...
                            trace(
                                ...
                                "[best={}, time_at_dest={}]\n",
                                k, loc{ tt_, l_idx }, new_best_[to_idx(l_idx)][v].time_,
                                adjusted_transfer_time(transfer_time_settings_, fp.duration()),
                                loc{ tt_, fp.target() }, new_best_[target][v].time_,
                                to_unix(time_at_dest_[k]));
                        }
        ...
        void update_intermodal_footpaths(unsigned const k) {
            ...
            state_.prev_station_mark_.for_each_set_bit([&](auto const i) {
                ...
                if (dist_to_end_[i] != std::numeric_limits<std::uint16_t>::max()) {
                    ...
                    if constexpr (Vias != 0U) {
                        ...
                        if (!new_tmp_[i][v].is_invalid() && is_via_[v][i]) {
                            auto const end_time = clamp(new_tmp_[i][v].time_  //
                            ...
                            trace_upd(
                                ...
                                "--({} +stay={})--> "
                                "({}, best={})",
                                k, loc{ tt_, l }, to_unix(new_tmp_[to_idx(l)][v].time_), dist_to_end_[i],
                                via_stops_[v].stay_,
                    ...
                    auto const tmp_time = tmp_bag.time_;
                    if (tmp_bag.is_invalid()) {
                    ...
                    trace_upd(
                        ...
                        "({}, best={})",
                        k, loc{ tt_, l }, to_unix(new_tmp_[to_idx(l)][Vias].time_), dist_to_end_[i],
                        loc{ tt_, location_idx_t{kIntermodalTarget} },
                        to_unix(new_best_[kIntermodalTarget][Vias].time_), to_unix(end_time));
                ...
                if (auto const it = td_dist_to_end_.find(l); it != end(td_dist_to_end_)) {
                    ...
                    auto const fp_start_time = new_tmp_[i][Vias].time_;
                    ...
                    if (fp.has_value()) {
                        ...
                        if (!new_best_[kIntermodalTarget][Vias].is_better(end_time)) {
                            ...
                            trace(
                                ...
                                k, loc{ tt_, l }, to_unix(fp_start_time), duration,
                                to_unix(end_time), new_best_[kIntermodalTarget][Vias].time_);
                        }
                    }
                }
                });
                        ...
                        if (et[v] && stp.can_finish<SearchDir>(is_wheelchair_)) {
                            ...
                            auto current_best =
                                get_best(round_times_[k - 1][l_idx][target_v],
                                    new_tmp_[l_idx][target_v].time_, new_best_[l_idx][target_v].time_);
                            ...
                            if (is_better(by_transport, time_at_dest_[k]) &&
                                ...
                                new_tmp_[l_idx][target_v].time_ =
                                    get_best(by_transport, new_tmp_[l_idx][target_v].time_);
            ...
            for (auto i = 0U; i != stop_seq.size(); ++i) {
                ...
                for (auto j = 0U; j != Vias + 1; ++j) {
                    ...
                    trace(
                        ...
                        to_unix(round_times_[k - 1][l_idx][v]), to_unix(new_best_[l_idx][v].time_),
                        to_unix(new_tmp_[l_idx][v].time_));
                    ...
                    if (et[v].is_valid() && stp.can_finish<SearchDir>(is_wheelchair_)) {
                        ...
                        if (is_better(by_transport, time_at_dest_[k]) &&
                            ...
                            trace(
                                ...
                                to_unix(new_best_[l_idx][target_v].time_), to_unix(new_tmp_[l_idx][target_v].time_),
                ...
                for (auto v = 0U; v != Vias + 1; ++v) {
                    ...
                    if (prev_round_time != kInvalid &&
                        ...
                        auto const new_et = get_earliest_transport(k, r, stop_idx, day, mam,
                            stp.location_idx());
                        current_best[v] = get_best(current_best[v], new_best_[l_idx][target_v].time_,
                            new_tmp_[l_idx][target_v].time_);
                        if (new_et.is_valid() &&
