...
struct simple_raptor {
  ...
  void add_start(location_idx_t const l, unixtime_t const t) {
    ...
    best_[to_idx(l)] = unix_to_delta(base(), t);
    round_times_[0U][to_idx(l)] = unix_to_delta(base(), t);
  ...
  void execute(unixtime_t const start_time,
    ...
    for (auto k = 1U; k != end_k; ++k) {
      for (auto i = 0U; i != n_locations_; ++i) {
        best_[i] = get_best(round_times_[k][i], best_[i]);
      }
      is_dest_.for_each_set_bit(
          [&](std::uint64_t const i) { update_time_at_dest(k, best_[i]); });
    ...
    is_dest_.for_each_set_bit([&](auto const i) {
      for (auto k = 1U; k != end_k; ++k) {
        auto const dest_time = round_times_[k][i];
        if (dest_time != kInvalid) {
          trace("ADDING JOURNEY: start={}, dest={} @ {}, transfers={}\n",
                start_time, delta_to_unix(base(), round_times_[k][i]),
...
private:
        ...
        if (is_better(fp_target_time, best_[target]) &&
          ...
          if (lower_bound == kUnreachable ||
            ...
            trace_upd(
                ...
                k, location{tt_, l_idx}, to_unix(tmp_[to_idx(l_idx)]),
                adjusted_transfer_time(transfer_time_settings_, fp.duration()),
                location{tt_, fp.target()}, best_[to_idx(fp.target())],
          ...
          trace_upd(
              ...
              k, location{tt_, l_idx}, to_unix(tmp_[to_idx(l_idx)]),
              adjusted_transfer_time(transfer_time_settings_, fp.duration()),
              location{tt_, fp.target()}, to_unix(best_[to_idx(fp.target())]),
              fp_target_time);

          ++stats_.n_earliest_arrival_updated_by_footpath_;
          round_times_[k][to_idx(fp.target())] = fp_target_time;
          ...
          trace(
              "┊ ├k={}   NO FP UPDATE: {} [best={}] --{}--> {} "
              "[best={}, time_at_dest={}]\n",
              k, location{tt_, l_idx}, to_unix(best_[to_idx(l_idx)]),
              fp.duration(), location{tt_, fp.target()},
              best_[to_idx(fp.target())], to_unix(time_at_dest_[k]));
        }
      }
    });
  }
        ...
        if (is_better(fp_target_time, best_[target]) &&
          ...
          if (lower_bound == kUnreachable ||
            ...
            trace_upd(
                ...
                k, location{tt_, l_idx}, to_unix(tmp_[to_idx(l_idx)]),
                fp.duration(), location{tt_, fp.target()},
                best_[to_idx(fp.target())], fp_target_time, lower_bound,
                to_unix(clamp(fp_target_time + dir(lower_bound))),
                to_unix(time_at_dest_[k]));
          ...
          trace_upd(
              ...
              k, location{tt_, l_idx}, to_unix(tmp_[to_idx(l_idx)]),
              fp.duration(), location{tt_, fp.target()},
              to_unix(best_[to_idx(fp.target())]), fp_target_time);
      ...
      if (state_.prev_station_mark_[i] || state_.station_mark_[i]) {
        ...
        if (dist_to_end_[i] != std::numeric_limits<std::uint16_t>::max()) {
          ...
          if (is_better(end_time, best_[kIntermodalTarget])) {
            ...
            best_[kIntermodalTarget] = end_time;
  ...
  bool update_rt_transport(unsigned const k, rt_transport_idx_t const rt_t) {
    ...
    for (auto i = 0U; i != stop_seq.size(); ++i) {
      ...
      auto const prev_round_time = round_times_[k - 1][l_idx];
  ...
  bool update_route(unsigned const k, route_idx_t const r) {
    ...
    for (auto i = 0U; i != stop_seq.size(); ++i) {
      ...
      trace(
          ...
          to_unix(round_times_[k - 1][l_idx]), to_unix(best_[l_idx]),
          to_unix(tmp_[l_idx]));
      ...
      if (et.is_valid() && stp.can_finish<SearchDir>(is_wheelchair_)) {
        ...
        if (is_better(by_transport, current_best) &&
          ...
          ++stats_.n_earliest_arrival_updated_by_route_;
          round_times_[k][l_idx] = by_transport;
          state_.station_mark_.set(l_idx, true);
          current_best = by_transport;
          any_marked = true;
        } else {
          trace(
              ...
              to_unix(by_transport), to_unix(round_times_[k - 1][l_idx]),
              to_unix(best_[l_idx]), to_unix(tmp_[l_idx]),
