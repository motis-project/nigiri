...
struct cost_func_t {
  ...
  std::size_t operator ()(std::pair<segment_idx_t,duration_t> const& element) const {
...
struct a_star_stats {
  std::map<std::string, std::uint64_t> to_map() const {
    return {
          {"max_transfers_reached_", max_transfers_reached_},
...
struct tb_a_star{
  ...
  explicit tb_a_star(std::size_t const& init_cap) :
    ...
    pred_ = vector_map<segment_idx_t,segment_idx_t>(init_cap,segment_idx_t::invalid());
    day_idx_ = vector_map<segment_idx_t,day_idx_t>(init_cap);
    transfers_ = vector_map<segment_idx_t,uint8_t>(init_cap,std::numeric_limits<uint8_t>::max());
