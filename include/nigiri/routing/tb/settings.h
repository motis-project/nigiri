...
// Same horizon as RAPTOR: waiting (at the origin or at a transfer) and
// travelling are only bounded by kMaxTravelTime.
constexpr auto const kTBMaxTravelTimeDays =
    static_cast<unsigned>(kMaxTravelTime / std::chrono::days{1});
// calendar days searched for the first departure, as in RAPTOR
constexpr auto const kTBNDaysToIterate =
    static_cast<day_idx_t::value_t>(kTBMaxTravelTimeDays + 1U);
// longest transfer (waiting time) generated by preprocess()
constexpr auto const kTBMaxTransferTime =
    std::chrono::duration_cast<i32_minutes>(kMaxTravelTime);
