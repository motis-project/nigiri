...
struct equal_str {
  ...
  bool operator()(std::string const& a, char const* b) const { return a == b; }

  bool operator()(char const* a, char const* b) const { return a == b; }

  bool operator()(std::string_view const& a, char const* b) const {
    return a == b;
  }

  template <std::size_t N>
  bool operator()(const char (&b)[N], std::string_view const& a) const {
    return a == b;
  }

  template <std::size_t N>
  bool operator()(const char (&b)[N], std::string const& a) const {
    return a == b;
  }
};
...
struct hash_str {
  ...
  template <std::size_t N>
  std::size_t operator()(const char (&x)[N]) const {
    return std::hash<std::string_view>()(x);
  }
};
