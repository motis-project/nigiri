...
struct service {
  ...
  using direction_info =
      std::variant<utl::cstr /* custom string */, int /* eva number */>;
  ...
  service(parser_info origin, int num_repetitions, int interval,
  ...
  int find_first_stop_at(int eva_num) const;

  int get_first_stop_index_at(int eva_num) const;
  ...
  vector<tz_offsets> get_stop_timezones(std::map<int, tz_offsets> const&) const;
...
void expand_traffic_days(service const& s, bitfield_map_t const& bitfields,
                         Fn&& consumer) {
  struct split_info {
    bitfield traffic_days_;
...
template <typename Fn>
void to_local_time(
    std::map<int, tz_offsets> const& timezones,
    std::pair<std::chrono::sys_days, std::chrono::sys_days> const& interval,
    service const& s, Fn&& consumer) {
...
template <typename Fn>
void parse_services(
    config const& c, char const* filename, bitfield_map_t const& bitfields,
    std::pair<std::chrono::sys_days, std::chrono::sys_days> const& interval,
    std::map<int, tz_offsets> const& timezones, std::string_view file_content,
    std::function<void(std::size_t)> bytes_consumed, Fn&& consumer) {
