...
inline std::tuple<duration_t, int, bool> local_mam_to_utc_mam(
    tz_offsets const& tz,
    unixtime_t const day,
  ...
  auto const utc_mam = initial_day_shift * 1_days + local_mam + offset;

  auto const local_time = day + (initial_day_shift * 1_days) + local_mam;
  ...
  if (utc_mam < duration_t{0}) {
    return {utc_mam + duration_t{1440}, -1, valid};
  } else {
    return {utc_mam, 0, valid};
  }
}
