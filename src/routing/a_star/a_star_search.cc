...
// In the "Praktikum Algorithmen" lab, a journey can't take more than 24h (= 1440 min).
// On top of that, there are certainly not more than 10 transfers.
...
a_star_search::a_star_search(
  ...
  // Analogously to the constructor in query_engine.cc for tb, 'state_.end_reachable_'
  // and 'state_.dist_to_dest_' are built here.
  state_.reset();
  auto const mark_dest_segments = [&](location_idx_t const l, duration_t const d) {
  ...
  if (dist_to_dest.empty()) {
    ...
    is_dest_.for_each_set_bit([&](std::size_t const i) {
      ...
      for (auto const fp : tt_.locations_.footpaths_in_[state_.tbd_.prf_idx_][l]) {
...
void a_star_search::execute(unixtime_t start_time, uint8_t max_transfers, unixtime_t worst_time_at_dest, profile_idx_t prf_idx, pareto_set<journey>& journeys) {
  journey_found_ = false;
  start_time_ = start_time;
  // In the "Praktikum Algorithmen" lab, a journey can't take more than 24h (= 1440 min).
  // On top of that, there are certainly not more than 10 transfers.
  cost_upper_bound_ = std::min(1440, (worst_time_at_dest - start_time).count())
  ...
  while (!open_set_.empty()) {
    ...
    if (arrival_times_[current] < worst_time_at_dest && num_transfers_until_segment_[current] < max_transfers) {
      expand_node(current);
    }
  }
  ...
  if (journey_found_) {
    ...
    auto total_transfers = static_cast<uint8_t>(num_transfers_until_segment_[end_segment_]);
    journeys.add(journey{.start_time_=start_time, .dest_time_=arrival_times_[end_segment_] + offset, .transfers_=total_transfers});
  }
}
  ...
  for (int i=0; i < path_segments.size(); i++) {
    ...
    if (i == 0) {
      if (q.start_match_mode_ == location_match_mode::kIntermodal) {
        // *** first leg ***
        // We may only search an offset with 'j.start_time_ + o.duration() <= get_departure_time(seg)' instead of ==, because there
        // doesn't have to be an offset that starts exactly at the start time. To make sure that the total journey duration is still
        // displayed correctly, we update 'j.start_time_' in the line after that.
        auto const offset_it =
          utl::find_if(q.start_, [&](routing::offset const& o) {
            return o.target() == start_location && j.start_time_ + o.duration() <= get_departure_time(seg);
          });
        j.start_time_ = get_departure_time(seg) - offset_it->duration();
        j.add(journey::leg(direction::kForward, get_special_station(special_station::kStart), start_location,
          get_departure_time(seg)-offset_it->duration(), get_departure_time(seg), *offset_it));
      } else {
        for (auto const fp : tt_.locations_.footpaths_in_[state_.tbd_.prf_idx_][start_location]) {
    ...
    auto const rel = (seg - first);  // relative difference of requested segment compared to first segment of this trip
    auto const from_stop = static_cast<stop_idx_t>(to_idx(rel));
    auto const to_stop = static_cast<stop_idx_t>(from_stop + 1U);

    // If first segment of journey on current trip, we update the first location/start variables.
    if (i == 0 || state_.tbd_.segment_transports_[seg] != state_.tbd_.segment_transports_[path_segments[i-1]]) {
      first_location_on_current_trip = start_location;
      first_stop_on_current_trip = from_stop;
      first_segment_on_current_trip = seg;
    }

    // If last segment of journey on current trip, we combine all segments to one journey leg.
    if (i == path_segments.size()-1 || state_.tbd_.segment_transports_[seg] != state_.tbd_.segment_transports_[path_segments[i+1]]) {
      // ** (intermediate) run **
      j.add(journey::leg(direction::kForward, first_location_on_current_trip, end_location, get_departure_time(first_segment_on_current_trip), get_arrival_time(seg),
      journey::run_enter_exit(
          rt::run{.t_ = {state_.tbd_.segment_transports_[seg], segment_day_[seg]},
                          .stop_range_ = {0U, static_cast<stop_idx_t>(
                        tt_.route_location_seq_[tt_.transport_route_[transport_id]].size())}}, first_stop_on_current_trip, to_stop)));
      if (i != path_segments.size()-1) {
        // *** intermediate footpath ***
        j.add(journey::leg(direction::kForward, end_location, end_location, get_arrival_time(seg), get_arrival_time(seg)+tt_.locations_.transfer_time_[end_location],
        footpath(end_location, duration_t{std::chrono::duration<int, std::ratio<60>>(tt_.locations_.transfer_time_[end_location])})));
        num_transfers++;
      } else {
        // *** final leg ***
        auto const duration_to_dest = state_.dist_to_dest_.at(seg);
        if (duration_to_dest.count() == 0) {
          j.add(journey::leg(direction::kForward, end_location, end_location, get_arrival_time(seg), get_arrival_time(seg),
        footpath(end_location, duration_t{std::chrono::duration<int, std::ratio<60>>(0)})));
        } else {
          auto const offset_it = std::find_if(begin(q.destination_), end(q.destination_),
          [&](routing::offset const& o) {
            return o.target() == end_location && o.duration() == duration_to_dest;
          });   // analogous to tb implementation
          j.add(journey::leg(direction::kForward, end_location, get_special_station(special_station::kEnd), get_arrival_time(seg), get_arrival_time(seg)+duration_to_dest,
          *offset_it));
        }
      }
    }
...
void a_star_search::expand_node(tb::segment_idx_t current) {
  ...
  for (auto neighbor_transfer: state_.tbd_.segment_transfers_[current]) {
    ...
    auto new_neighbor_day = segment_day_[current] + neighbor_transfer.get_day_offset();
    if (state_.tbd_.bitfields_[neighbor_transfer.traffic_days_].test(to_idx(segment_day_[current]))) {
  ...
  if (state_.end_reachable_[current]) {
    ...
    open_set_.push(el);
    cost_upper_bound_ = std::min(cost_upper_bound_, cost_function(el, arrival_times_.at(el.seg),
                                           num_transfers_until_segment_.at(el.seg)));
  }
}


void a_star_search::progress_neighbor_segment(tb::segment_idx_t current, tb::segment_idx_t neighbor, day_idx_t neighbor_day, bool is_same_trip) {
  auto [from_location, to_location] = get_segment_locations(neighbor);
  if (closed_set_.test(neighbor) || lower_bounds_.at(to_idx(to_location)) == std::numeric_limits<uint16_t>::max()) {
    return;
  }

  auto const t = state_.tbd_.segment_transports_[neighbor];
  auto const to_stop_idx = static_cast<stop_idx_t>(to_idx(neighbor - state_.tbd_.transport_first_segment_[t]) + 1);
  auto new_neighbor_arr_time = tt_.event_time({t, neighbor_day}, to_stop_idx, event_type::kArr);
  ...
  auto new_neighbor_cost = cost_function(neighbor_element, new_neighbor_arr_time, new_neighbor_transfer_number);
  if (new_neighbor_cost >= cost_upper_bound_) {
    return;
  }
  // If the neighbor has already been reached before, and the path to the neighbor over current isn't better than the best known
  // path to the neighbor so far, we can skip this neighbor.
  if (arrival_times_.contains(neighbor) && new_neighbor_cost >=
    cost_function(neighbor_element, arrival_times_.at(neighbor), num_transfers_until_segment_.at(neighbor))) {
    return;
   }
...
size_t a_star_search::cost_function(open_set_element el, unixtime_t arrival_time, uint32_t num_transfers_until_segment) {
  if (el.type == element_type::SEGMENT) {
    auto to_location_idx = to_idx(to_location_.at(el.seg));
    return (arrival_time - start_time_).count() + ALPHA * num_transfers_until_segment + lower_bounds_.at(to_location_idx);
  } else {
    return (arrival_time - start_time_).count() + ALPHA * num_transfers_until_segment + state_.dist_to_dest_.at(el.seg).count();
  }
}


// This method returns start and the end location for a given segment.
pair<location_idx_t, location_idx_t> a_star_search::get_segment_locations(tb::segment_idx_t seg) {
...
unixtime_t a_star_search::get_arrival_time(tb::segment_idx_t seg) {
  auto const t = state_.tbd_.segment_transports_[seg];
  auto const to_stop_idx = static_cast<stop_idx_t>(to_idx(seg - state_.tbd_.transport_first_segment_[t]) + 1);
  auto arr_time = tt_.event_time({t, segment_day_[seg]}, to_stop_idx, event_type::kArr);
  return arr_time;
}

unixtime_t a_star_search::get_departure_time(tb::segment_idx_t seg) {
  auto const t = state_.tbd_.segment_transports_[seg];
  auto const from_stop_idx = static_cast<stop_idx_t>(to_idx(seg - state_.tbd_.transport_first_segment_[t]));
  auto dep_time = tt_.event_time({t, segment_day_[seg]}, from_stop_idx, event_type::kDep);
  return dep_time;
}
