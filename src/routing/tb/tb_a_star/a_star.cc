...
std::pair<std::vector<segment_idx_t>, bool> get_neighbours(
  ...
  for (auto const transfer : tbd.segment_transfers_[s_idx]) {
    if (tbd.bitfields_[transfer.traffic_days_].test(day.v_))
      neighbours.push_back(transfer.to_segment_);
  }
  auto next_exists =
      s_idx + 1 < tbd.segment_transports_.size() &&
      tbd.segment_transports_[s_idx] == tbd.segment_transports_[s_idx + 1];
...
unixtime_t get_time(segment_idx_t const& idx,
  ...
  auto const stop_idx =
      static_cast<stop_idx_t>((idx - tbd.transport_first_segment_[transport] +
                               (event == event_type::kArr ? 1 : 0))
                                  .v_);
...
location_idx_t get_location(segment_idx_t const& s,
                            timetable const& tt,
                            tb_data const& tbd,
                            event_type const& event) {
  auto transport = tbd.segment_transports_[s];
  auto stop_idx = s - tbd.transport_first_segment_[transport] +
                  (event == event_type::kArr ? 1 : 0);
...
duration_t tb_a_star::heuristic(segment_idx_t const& s) {
  return duration_t(
      travel_time_lower_bound_
          [get_location(s, tt_, state_.tbd_, event_type::kArr).v_]);
}
...
void tb_a_star::execute(unixtime_t const start_time,
  ...
  vector_map<segment_idx_t, bool> seen(state_.tbd_.segment_transfers_.size(),
                                       false);
  duration_t best_cost_at_dest = duration_t::max();
  unixtime_t arrival_time_limit =
      std::min(worst_time_at_dest, start_time + duration_t(1441));
  ...
  while (true) {
    ...
    auto [neighbours, next_exists] =
        get_neighbours(p, state_.tbd_, day_of_arrival);
    for (std::size_t i = 0; i < neighbours.size(); ++i) {
      ...
      auto neighbour_arrival = get_time(neighbour, tt_, event_type::kArr,
  ...
  algo_stats_.max_transfers_reached_ =
      transfers_[end_segment_] >= max_transfers;
  results.add({.legs_ = {},
               .start_time_ = start_time,
               .dest_time_ = get_time(end_segment_, tt_, event_type::kArr,
                                      state_.tbd_, day_idx_[end_segment_]) +
                             state_.dist_to_dest_[end_segment_],
...
segment_idx_t tb_a_star::get_departure_segment(segment_idx_t const& s) {
  ...
  while (p != invalid_segment && state_.tbd_.segment_transports_[p] ==
                                     state_.tbd_.segment_transports_[prev]) {
    prev = p;
    p = pred_[p];
  }
  return prev;
...
void tb_a_star::reconstruct(query const& q, journey& j) {
  if (is_invalid<segment_idx_t>(end_segment_))
    throw std::runtime_error("reconstruction demanded with no route found");
  auto const offset = state_.dist_to_dest_.at(end_segment_);
  auto arrival_location =
      get_location(end_segment_, tt_, state_.tbd_, event_type::kArr);
  ...
  if (q.dest_match_mode_ == location_match_mode::kIntermodal) {
    ...
    utl::verify(offset_it != q.destination_.end(),
                "Reconstruction failed because end_segment is invalid");
    j.legs_.emplace_back(direction::kForward, arrival_location,
                         get_special_station(special_station::kEnd),
                         get_time(end_segment_, tt_, event_type::kArr,
                                  state_.tbd_, day_idx_[end_segment_]),
                         j.arrival_time(), *offset_it);
    j.legs_.push_back(get_run_leg(end_segment_));
  } else {
    auto arr_time = get_time(end_segment_, tt_, event_type::kArr, state_.tbd_,
    ...
    auto const handle_fp = [&](footpath const& fp) {
      ...
      j.legs_.emplace_back(direction::kForward, arrival_location, fp.target(),
    ...
    if (!found_path) {
      for (auto const fp : tt_.locations_.footpaths_out_[state_.tbd_.prf_idx_]
                                                        [arrival_location]) {
        if (handle_fp(fp)) {
          found_path = true;
          break;
  ...
  while (!is_invalid<segment_idx_t>(p)) {
    ...
    j.legs_.emplace_back(direction::kForward, arr_l, j.legs_.back().from_,
                         arr_time, arr_time + fp.duration(), fp);
  ...
  if (q.start_match_mode_ == location_match_mode::kIntermodal) {
    ...
    utl::verify(offset_it != end(q.start_),
    ...
    j.legs_.emplace_back(direction::kForward,
                         ...
                         first_dep_l, first_dep_time - offset_it->duration(),
                         first_dep_time, *offset_it);
  } else {
    for (auto const fp :
      ...
      if (start_time + fp.duration() <= first_dep_time &&
        ...
        j.legs_.emplace_back(direction::kForward, fp.target(), first_dep_l,
                             first_dep_time - fp.duration(), first_dep_time,
                             fp);
        break;
      }
    }
