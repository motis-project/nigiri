...
routing_result pong(timetable const& tt,
  ...
  // Populate per-leg alternative transports (e.g. Flügelzug / coupled trains).
  // Uses find_equivalent_transports which is also called from reconstruct.cc,
  // ensuring consistency between PONG and rRAPTOR results.
  if (q.include_coupled_trips_) {
    for (auto& j : s_state.results_) {
      for (auto& leg : j.legs_) {
        ...
        leg.alternatives_ = find_equivalent_transports(
            tt, rtt, q.prf_idx_, leg.from_, leg.to_, leg.dep_time_,
            leg.arr_time_, ree.r_.t_.t_idx_);
      }
    }
  }
