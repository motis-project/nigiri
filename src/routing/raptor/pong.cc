...
routing_result pong(timetable const& tt,
  ...
  auto const get_result_count = [&](bool const include_too_slow) {
    return utl::count_if(*result.journeys_, [&](journey const& j) {
      ...
      // reconstruction can fail (error_) -- both get erased before
      // delivery and neither can ever be matched by a ping. counting
      // such undeliverable entries terminates the extension early and
      // starves real journeys of their validation (under-delivery, e.g.
      // across overnight service gaps).
