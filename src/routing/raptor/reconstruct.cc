...
std::optional<journey::leg> find_start_footpath(timetable const& tt,
  ...
  auto const& footpaths =
      kFwd ? tt.locations_.footpaths_in_[q.profile_][leg_start_location]
           : tt.locations_.footpaths_out_[q.profile_][leg_start_location];
    ...
    for (auto const& o : q.start_) {
      if (matches(tt, q.start_match_mode_, o.target(), leg_start_location) &&
        ...
        trace_start(
            ...
            "offset={}\n",
            location{tt, o.target()}, leg_start_time, j.start_time_,
            o.duration());
...
void reconstruct_journey(timetable const& tt,
  ...
  auto const get_legs =
      ...
      for (auto const& dest_offset : q.destination_) {
        ...
        for_each_meta(
            ...
            [&](location_idx_t const eq) {
              ...
              for (auto const& fp :
                   kFwd ? tt.locations_.footpaths_in_[q.profile_][eq]
                        : tt.locations_.footpaths_out_[q.profile_][eq]) {
    ...
    auto const fps = kFwd ? tt.locations_.footpaths_in_[q.profile_][l]
                          : tt.locations_.footpaths_out_[q.profile_][l];
    for (auto const& fp : fps) {
      trace_rc("FP: (name={}, id={}) --{}--> (name={}, id={})\n",
               tt.locations_.names_.at(l).view(),
               tt.locations_.ids_.at(l).view(), fp.duration().count(),
               tt.locations_.names_.at(fp.target()).view(),
               tt.locations_.ids_.at(fp.target()).view());
