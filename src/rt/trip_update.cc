...
//   //    transfers (all travel combinations are built) => several can be
//   active! for (auto i = lb; i != end(tt.trip_id_to_idx_) &&
//   id_matches(i->first); ++i) {
...
//       tt.bitfields_[tt.transport_traffic_days_[t]]; if
//       (traffic_days.test(to_idx(t_day))) {
...
trip::trip(timetable const&,
           rt_timetable&,
           trip_id const&,
           date::year_month_day const) {
