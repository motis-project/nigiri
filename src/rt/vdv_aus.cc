...
#define vdv_trace(...)
    ...
    std::optional<bool> get_opt_bool(
        ...
        return xpath != nullptr
                   ? utl::parse<bool>(xpath.node().child_value())
                   : default_to;
    }
    ...
    updater::vdv_stop::vdv_stop(nigiri::location_idx_t const l,
        ...
        : l_{l},
          ...
          in_forbidden_{
              is_vdv(f)
                  ? *get_opt_bool(n, "Einsteigeverbot", false)
                  : *get_opt_str(n,
          ...
          out_forbidden_{
              is_vdv(f)
                  ? *get_opt_bool(n, "Aussteigeverbot", false)
                  : get_opt_str(n,
          ...
          passing_through_{
              is_vdv(f)
                  ? *get_opt_bool(n, "Durchfahrt", false)
                  : in_forbidden_ && out_forbidden_
          },
          arr_canceled_{
              is_vdv(f)
                  ? *get_opt_bool(n, "AnkunftFaelltAus", false)
                  : *get_opt_bool(n, "Cancellation", false)
          },
          dep_canceled_{
              is_vdv(f)
                  ? *get_opt_bool(n, "AbfahrtFaelltAus", false)
                  : arr_canceled_
          } {
    }
    ...
    vector<updater::vdv_stop> updater::resolve_stops(pugi::xml_node const vdv_run) {
        ...
        auto const siri_stop_stop_selectors = {
                                ...
                                if (candidate == end(candidates)) {
                                    candidates.emplace_back(
                                        run{
                                            tr,
                                            interval{
                                                static_cast<stop_idx_t>(stop_idx),
    ...
    void monotonize(frun &fr, rt_timetable &rtt) {
        ...
        for (auto i = stop_idx_t{0U}; i != fr.stop_range_.size(); ++i) {
            auto const rs = fr[fr.stop_range_.size() - 1 - i];
            ...
            for (auto vdv_stop = cursor; vdv_stop != end(vdv_stops); ++vdv_stop) {
                if (vdv_stop->l_ != location_idx_t::invalid() &&
                    ...
                    if (matched_arr || matched_dep) {
                        ...
                        stp = stop{
                                    stop{stp}.location_idx(), in_allowed_update,
                                    out_allowed_update,
