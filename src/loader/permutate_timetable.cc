...
void permutate_bitvec_map(permutation_t<T> const& p, BV& bv) {
  if (bv.size() == 0U) {
    return;
  }
  auto permutated = BV{};
  permutated.resize(bv.size());
  for (auto i = T{0U}; i != bv.size(); ++i) {
    permutated.set(i, bv.test(p[i]));
  }
  bv = std::move(permutated);
}

template <typename T>
void permutate_bits(permutation_t<T> const& p,
                    bitvec& bv,
                    unsigned int const n_bits_per_object) {
  utl::verify(bv.size() % n_bits_per_object == 0U,
              "unexpected bitvec dimensions");
  auto const n_elements = bv.size() / n_bits_per_object;
...
void permutate_route_times(
    route_permutation_t const& p,
...
cartesian_t get_cartesian(geo::latlng const& ll) {
  ...
  auto const lat_rad = ll.lat() * PI / 180;
  auto const lng_rad = ll.lng() * PI / 180;

  return {std::cos(lat_rad) * std::cos(lng_rad),
          std::cos(lat_rad) * std::sin(lng_rad), std::sin(lat_rad)};
}
...
vector_map<route_idx_t, geo::latlng> get_route_centroids(timetable const& tt) {
  ...
  for (auto r = route_idx_t{0U}; r < tt.n_routes(); ++r) {
    ...
    for (auto i = 0U; i < n; ++i) {
      ...
      auto const p = tt.locations_.coordinates_[loc];
      auto const [x, y, z] = get_cartesian(p);
      sum_x += x;
      sum_y += y;
      sum_z += z;
    }

    cartesian_t const avg = {
        sum_x / n,
        sum_y / n,
        sum_z / n,
    };
    auto const& [centroid_x, centroid_y, centroid_z] = avg;
...
vector_map<route_idx_t, geo::latlng> get_route_coordinates(
    timetable const& tt) {
  if constexpr (kRoutePermutationStrategy ==
...
std::pair<route_permutation_t, route_permutation_t> get_route_permutation(
    timetable const& tt) {
  if (tt.n_routes() == 0U) {
    return {};
  }
  ...
  auto const route_coords = get_route_coordinates(tt);

  std::stable_sort(
      begin(p), end(p), [&](route_idx_t const a, route_idx_t const b) {
        return morton_encode(route_coords[a]) < morton_encode(route_coords[b]);
      });
...
std::pair<transport_permutation_t, transport_permutation_t>
get_transport_permutation(vector_map<route_idx_t, interval<transport_idx_t>>&
                              route_transport_ranges) {
  if (route_transport_ranges.empty()) {
    return {};
  }
  auto const n_routes = route_transport_ranges.size();
...
std::pair<location_permutation_t, location_permutation_t>
get_location_permutation(timetable const& tt) {
  ...
  std::stable_partition(
      begin(p) + first_idx, end(p),
      [&](location_idx_t const l) { return !has_any_events(tt, l); });
...
void permutate_locations(timetable& tt) {
  ...
  permutate(p, locations.platform_codes_);
  permutate(p, locations.stop_codes_);
  permutate_bitvec_map(p, locations.ticketing_unavailable_);
...
void permutate_routes_and_transports(timetable& tt) {
  ...
  for (auto f = 0U; f != route_flag::kNumRouteFlags; ++f) {
    permutate_bits(p, tt.route_flags_[f], 2);
    permutate(p, tt.route_flags_per_section_[f]);
  }
  ...
  // ---------------------------
  auto const [pt, rt] = get_transport_permutation(tt.route_transport_ranges_);
  for (auto trip = trip_idx_t{0U}; trip != tt.trip_transport_ranges_.size();
       ++trip) {
    for (auto& tr : tt.trip_transport_ranges_[trip]) {
      tr.first = rt[tr.first];
    }
  }
