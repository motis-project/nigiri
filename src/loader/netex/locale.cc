...
netex_locale parse_locale(timetable& tt,
                          netex_data& data,
                          netex_ctx const& ctx,
  ...
  if (!locale.tz_name_.empty()) {
    locale.tz_idx_ = get_tz_idx(tt, data, locale.tz_name_);
    ...
    locale.tz_idx_ = get_tz_idx(tt, data, guessed_tz_name);
  }

  return locale;
}

std::optional<netex_locale> get_default_locale(timetable& tt,
                                               netex_data& data,
  ...
  return config.default_tz_.empty()
             ...
             : std::optional{netex_locale{
                   ...
                   .tz_idx_ = get_tz_idx(tt, data, config.default_tz_)}};
}
