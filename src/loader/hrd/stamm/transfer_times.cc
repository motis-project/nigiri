...
void parse_transfer_time_rules(transfer_times& r,
  ...
  auto const add_global_admin = [&](utl::cstr const s, provider_idx_t const p) {
  ...
  utl::for_each_line(utl::cstr{line_file_content}, [&](utl::cstr const line) {
    ...
    auto rule = line_rule{};
    rule.from_ =
        parse_side(line.substr(col::l_admin_1), line.substr(col::l_cat_1),
                   line.substr(col::l_line_1), line.substr(col::l_dir_1),
                   global, rule.n_wildcards_);
    rule.to_ = parse_side(line.substr(col::l_admin_2),
                          line.substr(col::l_cat_2), line.substr(col::l_line_2),
                          line.substr(col::l_dir_2), global, rule.n_wildcards_);
    rule.time_ = parse_time(line.substr(col::l_time));
    rule.guaranteed_ = line.len > col::l_guaranteed.from &&
                       line[col::l_guaranteed.from] == '!';
    if (global) {
  ...
  log(log_lvl::info, "loader.hrd.transfer_times",
      ...
      r.station_.size(), r.default_.has_value() ? r.default_->count() : -1,
...
event_attrs from_section(service const& s, std::size_t const sec_idx) {
  ...
  e.admin_ = b2e.admin_.has_value() ? *b2e.admin_
  ...
  auto const line =
      b2e.line_.has_value()
          ? *b2e.line_
          : (sec != nullptr && sec->line_.has_value() ? *sec->line_
                                                      : utl::cstr{});
  if (line.valid()) {
...
void scan_transfer_events(config const& c,
  ...
  auto const maybe_relevant = [&](specification const& spec) {
    for (auto const& stop_line : spec.stops_) {
      if (stop_line.len >= 7U && r.rule_stations_.contains(parse_eva_number(
                                     stop_line.substr(utl::field{0, 7})))) {
        return true;
      }
    }
    if (has_global) {
...
resolved_transfer resolve_time(station_rules const& sr,
  ...
  auto const find = [&](rule_tier const t) -> std::optional<std::size_t> {
    ...
    auto const hi = encode_side(
        static_cast<rule_tier>(static_cast<std::uint64_t>(t) + 1U), 0U, false);
...
void build_transfer_groups(stamm& st,
  ...
  for (auto& [base, station] : groups.stations_) {
    ...
    // classify every member the way build_hubs will: a slower cell in its
    // row means something slow starts there, one in its column means
    // something slow leads there, and a diagonal above the default means the
    // member is slow itself (its own re-boarding buffer). the derivation rule
    // lives in loader/transfer_rules.h, shared with the other loaders.
    auto members = std::vector<member_flags>(n_classes);
    for (auto a = std::size_t{0U}; a != n_classes; ++a) {
      for (auto b = std::size_t{0U}; b != n_classes; ++b) {
        if (a != b && at(class_rep[a], class_rep[b]).time_ > default_time) {
          members[a].slow_from_ = true;
          members[b].slow_to_ = true;
        }
      }
      members[a].slow_ = class_locations[a] != base &&
                         at(class_rep[a], class_rep[a]).time_ > default_time;
      n_slow += members[a].slow_ ? 1U : 0U;
    }

    // emit every cell the hubs cannot derive: the deviating and guaranteed
    // ones, and the default cells whose source or target is classified out
    ...
    for (auto a = std::size_t{0U}; a != n_classes; ++a) {
      for (auto b = std::size_t{0U}; b != n_classes; ++b) {
        ...
        if (c.time_ == default_time && !c.preferred_ &&
            derivable(members[a], members[b])) {
          continue;  // a hub of this base derives it
        }
  ...
  log(log_lvl::info, "loader.hrd.transfer_groups",
      ...
      "{} slow members",
      n_virt_locations, n_stations, n_edges, n_slow);
}
