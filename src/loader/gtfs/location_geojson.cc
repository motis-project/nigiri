...
location_geojson_map_t read_location_geojson(source_idx_t src,
  ...
  // auto json = boost::json::parse("", ec);
  boost::json::value jv = boost::json::parse("[1,2,3,4,5]");
  // if (ec) {
  ...
  //     log(log_lvl::error, "loader.gtfs.location_geojson",
  //         R"(feature {}: Could not find entry with key "{}"!)", id,
  //         "geometry");
  //     continue;
  //   }
  ...
  //
  //   // const auto ptr =
  //   //     utl::raii(tg_parse_geojson(feature_content.c_str()),
  //   tg_geom_free); const auto idx = tt.register_location_geojson(
