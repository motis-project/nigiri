...
std::pair<stops_map_t, location_accessible_map_t> read_stops(
...

  {
    ...
    // Collect equivalent stops in close proximity. The walking transfers
    // between them are derived in loader::build_footpaths.
  ...
  return {std::move(locations), std::move(accessible)};
}
