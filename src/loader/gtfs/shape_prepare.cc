...
auto get_closest(geo::latlng coordinate, std::span<geo::latlng const> const& shape) {
    if (shape.size() < 2) {
        return 0U;
    }
    ...
    auto const offset = geo::distance(coordinate, segment_from) <= geo::distance(coordinate, segment_to)
        ? segment : segment + 1;
    return static_cast<unsigned>(offset);
}

constexpr std::vector<location_idx_t> get_interior_locations(stop_seq_t const& stops) {
...
constexpr trip_idx_t find_in_cache(hash_map<shape_idx_t, std::vector<std::pair<std::vector<location_idx_t>, trip_idx_t>>> const& offsets_cache, shape_idx_t shape_index, std::vector<location_idx_t> const& locations) {
...
constexpr std::vector<shape_offset_t> split_shape(timetable const& tt, std::span<geo::latlng const> shape, std::vector<location_idx_t> const& locations) {
    if (shape.empty()) {
        return {};
    }
    ...
    for (auto const location_index : locations) {
        ...
        offset += get_closest(location.pos_, shape.subspan(static_cast<std::size_t>(offset.v_)));
        offsets[++index] = offset;
    }
    return offsets;
}

void calculate_shape_offsets(timetable const& tt, shapes_storage& shapes_data, vector_map<gtfs_trip_idx_t, trip> const& trips) {
    auto offsets_cache = hash_map<shape_idx_t, std::vector<std::pair<std::vector<location_idx_t>, trip_idx_t>>>{};
    for (auto const& trip : trips) {
        ...
        if (auto cached_trip_index = find_in_cache(offsets_cache, shape_index, locations); cached_trip_index != trip_idx_t::invalid()) {
            ...
            if (auto const it = offsets_cache.find(shape_index); it == offsets_cache.end()) {
                offsets_cache.emplace_hint(it, shape_index, std::vector{std::make_pair(locations, trip_index)});
            } else {
