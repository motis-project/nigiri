...
struct range {
  range() = default;
  range(std::vector<service::stop> const& stops, utl::cstr from_eva_or_idx,
...
private:
  ...
  int get_index(std::vector<service::stop> const& stops, utl::cstr eva_or_idx,
                utl::cstr hhmm_or_idx, bool is_departure_event) {
...
service::stop parse_stop(utl::cstr stop) {
  return {
      parse_verify<int>(stop.substr(0, utl::size(7))),
...
bool specification::read_line(utl::cstr line, char const* filename,
                              int line_number) {
  if (line.len == 0) {
    return false;
  }
...
service::service(parser_info origin, int num_repetitions, int interval,
...
int service::find_first_stop_at(int eva_num) const {
...
int service::get_first_stop_index_at(int eva_num) const {
...
vector<tz_offsets> service::get_stop_timezones(
    std::map<int, tz_offsets> const& tz) const {
  auto i = 0U;
